...
onload = function() {
  ...
  // 画像を読み込む
  img_kakko = new Image();
  img_kakko.src = "./img/kakko.png";

  img_a = new Image();
  img_a.src = "./img/a.png";

  img_aiue = new Image();
  img_aiue.src = "./img/aiue.png";

  img_mona = new Image();
  img_mona.src = "./img/mona.jpg";
...
function calculate() {
  ...
  determinant = mat_a * mat_d - mat_b * mat_c;
...
// 座標軸の描画
/////////////////////////////////
function draw_coordinates2(){

  if( mode == 4 ){
    if( Math.abs(determinant) < near0 ){
      return;
    }
    ctx.setTransform(mat_a, -mat_c, -mat_b, mat_d, width/2, height/2);
  }
  ...
  ctx.fillStyle = "rgb(0, 0, 0)";
  ctx.fillText(str,width/2-9,height/2+12);
...
function draw_monalisa(){
  ...
  ctx.globalAlpha = 0.9;
  ctx.setTransform(mat_a, -mat_c, -mat_b, mat_d, width/2, height/2);
  ctx.drawImage(img_mona, -img_mona.width/4, -img_mona.height/4, img_mona.width/2, img_mona.height/2 );
  ctx.globalAlpha = 1.0;
...
function draw_hiragana(){
  ...
  ctx.globalAlpha = 0.9;
  ctx.setTransform(mat_a, -mat_c, -mat_b, mat_d, width/2, height/2);
  ctx.drawImage(img_a, 0, -img_a.height );

  // 元に戻す
  ctx.setTransform(1, 0, 0, 1, 0, 0);

  ctx.globalAlpha = 0.1;
  ctx.drawImage(img_a, width/2, height/2-img_a.height );
  ctx.globalAlpha = 1.0;
...
function draw_hiragana2(){
  ...
  ctx.setTransform(mat_a, -mat_c, -mat_b, mat_d, width/2, height/2);
  ctx.drawImage(img_aiue, -img_aiue.width/2, -img_aiue.height/2 );
...
function disp_numbers() {
  ...
  ctx.drawImage(img_kakko, width-92, 11);

  str = "A =";
  ctx.font = "16px sans-serif";
  ctx.fillText(str,width-127, 40);

  str = ""+mat_a.toFixed(2);
  ctx.font = "13px sans-serif";
  tm = ctx.measureText(str);
  ctx.fillText(str,width-55-tm.width, 40-13);

  str = ""+mat_b.toFixed(2);
  ctx.font = "13px sans-serif";
  tm = ctx.measureText(str);
  ctx.fillText(str,width-20-tm.width, 40-13);
  ...
  str = "|A| =";
  ctx.font = "16px sans-serif";
  ctx.fillText(str,width-130, 90);

  str = ""+determinant.toFixed(2);
  ctx.font = "16px sans-serif";
  tm = ctx.measureText(str);
  ctx.fillText(str,width-50-tm.width, 90);
...
function regist_buttons() {
  ...
  // [M] モードボタン
  btn[13] = new button( 640-20-80, top + 20, 80, 40, 5 );

  // [X] ボタン
  btn[14] = new button( 320-20, 500, 40, 40, 5 );
...
function show_button( n, f){
  ...
  if( n<4 ){ // 緑
    ...
    if( f==1 ){
      ctx.fillStyle = "rgb(230, 230, 180)"; // 光る
    } else {
